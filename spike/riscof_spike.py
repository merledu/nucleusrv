...
class spike(pluginTemplate):
    ...
    def runTests(self, testList, cgf_file=None):
        ...
        for file in testList:
            ...
            # execute += " ; python3 -c \"import sys; f='{0}'; l=open(f).readlines(); open(f, 'w').write(''.join(l[1:]))\"".format(sig_file)
            execute += " ; python3 -c \"import sys; f='{0}'; " \
                       "lines = open(f).readlines()[1:]; " \
                       "marker = '6f5ca309'; " \
                       "idx = next((i for i, line in enumerate(lines) if marker in line), len(lines)); " \
                       "open(f, 'w').write(''.join(lines[:idx]))\"".format(sig_file)
