...
class nucleusrv(pluginTemplate):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # self.dut_path = [config['PATH'] if 'PATH' in config else ""][0]
        ...
        self.dut_path = os.path.dirname(self.pluginpath)
    ...
    def runTests(self, testList):
	  ...
	  # echo statement.
          if self.target_run:
            ...
            step01_parse_assembly   = f"python3 {self.dut_path}/tools/trace/scripts/assemblyParser.py --asm {testentry['work_dir']}/nucleusrv.disass --hex {testentry['work_dir']}/nucleusrv.hex"
            step02_clean_hex        = f"python3 {self.dut_path}/tools/trace/scripts/hex_cleaner.py --hex {testentry['work_dir']}/nucleusrv.hex"
            step03_go_to_nrv_root   = f"cd {self.dut_path}"
            step04_trigger_sim      = f"make sim IMEM={testentry['work_dir']}/nucleusrv.hex"
            step05_echo_msg         = f"echo Generating log..."
            step06_cleanup_trace    = f"python3 {self.dut_path}/tools/trace/scripts/sbtToLog.py --asm {testentry['work_dir']}/nucleusrv.disass --sbt_dump {self.dut_path}/trace.log --log {testentry['work_dir']}/nucleusrv.log"
            step07_cnvrt_log_to_sig = f"python3 {self.dut_path}/tools/trace/scripts/logToSignature.py --log {testentry['work_dir']}/nucleusrv.log --disass {testentry['work_dir']}/nucleusrv.disass --signature {testentry['work_dir']}/DUT-nucleusrv.signature" 
          else:
            simcmd = 'echo "NO RUN"'
        ...
        #   execute = '@cd {0}; {1}; {2};'.format(testentry['work_dir'], cmd, simcmd)
          ...
          self.objdump = f"riscv32-unknown-elf-objdump -D {testentry['work_dir']}/{elf} > {testentry['work_dir']}/nucleusrv.disass"
          print(self.objdump)

          execute = f"@cd {testentry['work_dir']}; {cmd}; {self.objdump}; {step01_parse_assembly}; {step02_clean_hex}; {step03_go_to_nrv_root}; {step04_trigger_sim}; {step05_echo_msg}; cd {testentry['work_dir']}; {step06_cleanup_trace}; {step07_cnvrt_log_to_sig}"
