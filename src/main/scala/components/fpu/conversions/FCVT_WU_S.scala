...
class FCVT_WU_S extends Module with Parameters {
  val io: FCVT_WU_S_IO = IO(new FCVT_WU_S_IO)

  //val fNToInt: RecFNToIN = Module(new RecFNToIN(8, 24, 32))

  //Seq(
  //  (fNToInt.io.in, recFNFromFN(8, 24, io.in)),
  //  (fNToInt.io.roundingMode, 0.U),
  //  (fNToInt.io.signedOut, 0.B)
  //).map(f => f._1 := f._2)

  //io.out := fNToInt.io.out
