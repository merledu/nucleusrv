...
class ImmediateGen(implicit val config:Configs) extends Module {
  ...
  when(
    ...
    val ext_i = Cat(Fill(config.XLEN-12, imm_i(11)), imm_i)
    io.out := ext_i
  ...
  //U-type
    .elsewhen(opcode === 23.U || opcode === 55.U) {
      ...
      val ext_u = Cat(imm_u, Fill(config.XLEN-20, 0.U))
      io.out := ext_u
    }
    //S-type
    .elsewhen(opcode === 35.U) {
      val imm_s = Cat(io.instruction(31, 25), io.instruction(11, 7))
      val ext_s = Cat(Fill(config.XLEN-12, imm_s(11)), imm_s)
      io.out := ext_s
    }
    //SB-type
    .elsewhen(opcode === 99.U) {
      ...
      val ext_sb = Cat(Fill(config.XLEN-13, imm_sb(11)), imm_sb, 0.U)
      io.out := ext_sb
    }
    //UJ-type
    .otherwise //(opcode === 111.U)
  {
    ...
    val ext_uj = Cat(Fill(config.XLEN-21, imm_uj(19)), imm_uj, 0.U)
    io.out := ext_uj
  }
}
