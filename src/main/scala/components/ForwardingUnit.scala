...
class ForwardingUnit extends Module {
  val io = IO(new Bundle {
    ...
    val ex_reg_vins = Input(Bool())
    val mem_reg_vins = Input(Bool())
  ...
  // && io.id_reg_instruction(14,12) === "b100".U
  
// for rs1 whenver we need opivx so we check previous vd and scalr reg write 
// for  

  when(io.reg_rs1 === io.ex_reg_rd && io.ex_reg_rd =/= 0.U && (io.ex_regWrite || io.ex_reg_vset ) && io.v_ins === 0.B){
    io.forwardA := 1.U
  }.elsewhen(io.reg_vs1 === io.ex_reg_vd && io.ex_reg_vd =/= 0.U && io.ex_reg_write && io.v_ins === 1.B && io.id_reg_instruction(14,12) =/= "b100".U){
    io.forwardA := 1.U
  }.elsewhen(io.reg_rs1 === io.mem_reg_rd && io.mem_reg_rd =/= 0.U && (io.mem_regWrite || io.mem_reg_vset) && io.v_ins === 0.B){ 
    io.forwardA := 2.U
...
//vset 
    ...
    io.forwardA := 1.U 
  }.elsewhen(io.reg_vs1 === io.mem_reg_vd && io.mem_reg_vd =/= 0.U && io.mem_regWrite  && io.v_ins === 1.B && io.id_reg_instruction(14,12) === "b100".U ){
    io.forwardA := 2.U
