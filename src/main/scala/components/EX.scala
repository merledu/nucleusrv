...
class Execute extends Module {
  val io = IO(new Bundle {
    ...
    val readData1 = Input(UInt(64.W))
    val readData2 = Input(UInt(64.W))
    val pcAddress = Input(UInt(64.W))
    val func7 = Input(UInt(1.W))
    val func3 = Input(UInt(3.W))
    val mem_result = Input(UInt(64.W))
  ...
  val aluIn2 = Mux(io.ctl_aluSrc, inSelMux, io.immediate)
