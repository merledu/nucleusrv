...
class MDUWrapperTest extends FreeSpec with ChiselScalatestTester {
    "MDU Wrapper Test" in {
        implicit val config = WishboneConfig(32,32)
        test(new MDUWrapper(new WBRequest(), new WBResponse())).withAnnotations(Seq(VerilatorBackendAnnotation)){ c =>
