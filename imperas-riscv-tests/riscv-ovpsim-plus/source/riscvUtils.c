...
memEndian riscvGetDataEndian(riscvP riscv, riscvMode mode) {
    ...
    memEndian result = riscv->configInfo.endian;
...
//
// Return a mask of extensions that are enabled and implicit or can only be
// enabled implicitly (not visible in misa)
//
riscvArchitecture riscvGetImplictExtensionMask(riscvConfigCP cfg) {

    riscvArchitecture result = cfg->archImplicit;
