...
typedef enum riscvDisableReasonE {
    ...
    RVD_ACTIVE     = 0x000, // running
    RVD_WFI        = 0x001, // waiting in WFI
    RVD_RESET      = 0x002, // waiting in reset
    RVD_DEBUG      = 0x004, // waiting for debug
    RVD_WRS        = 0x008, // waiting for reservation set (wrs.*)
    RVD_STO        = 0x010, // waiting for reservation set (wrs.sto)
    RVD_INT        = 0x020, // waiting for any pending and enabled interrupt
    RVD_CUSTOM_WFI = 0x040, // waiting for custom reason with WFI semantics
    RVD_CUSTOM_NMI = 0x080, // waiting for custom reason with NMI semantics
    RVD_CUSTOM_HLT = 0x100, // waiting for custom reason (persists over reset)

    // legacy disable reasons
    RVD_CUSTOMI = RVD_CUSTOM_WFI,

    // states from which to restart
    RVD_RESTART_WFI   = RVD_WFI|RVD_WRS|RVD_STO|RVD_INT|RVD_CUSTOM_WFI,
