...
typedef struct riscvS {
    ...
    Bool               restartFromWFI:1;// always restart from WFI state
    ...
    Bool               NMI           :1;// whether NMI input is pending
    ...
    Bool               disableNMI;      // whether NMI disabled
    ...
    vmiModelTimerP     wfiTimer;        // artifact WFI restart timer
    ...
    Uns8               eventState[32];  // mhpmevent status bits
...
//
// Is Sscofpmf configured?
//
inline static Bool Sscofpmf(riscvP riscv) {
    return riscv->configInfo.Sscofpmf;
}
...
// Is NMI present?
//
inline static Bool nmiPresent(riscvP riscv) {
    return !riscv->configInfo.nmi_absent;
}
