...
//
#define RISCV_UPDATE_DISABLE_NMI_FN(_NAME) void _NAME( \
    riscvP riscv,               \
    Bool   disable              \
)
typedef RISCV_UPDATE_DISABLE_NMI_FN((*riscvUpdateDisableNMIFn));
...
//
// Implementation-specific NMI actions (NOTE: this callback must set mcause or
// mncause, depending on whether RNMI is implemented, to a state consistent
// with the NMI that is about to be taken)
//
...
//
// Perform morph-time callback to check that memory access using the given
// base register and offset is legal
//
#define RISCV_CHECK_MEM_MT_FN(_NAME) void _NAME( \
    riscvP riscv,               \
    vmiReg base,                \
    Int32  offset,              \
    Uns32  bits,                \
...
typedef RISCV_CHECK_MEM_MT_FN((*riscvCheckMemMtFn));
