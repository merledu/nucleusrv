...
typedef enum riscvParamVariantE {
    ...
    RVPV_NMI       = (1ULL<<24),    // present if NMI implemented
    RVPV_RNMI      = (1ULL<<25),    // present if RNMI implemented
    RVPV_CLIC      = (1ULL<<26),    // present if CLIC enabled
    RVPV_CLUSTER   = (1ULL<<27),    // present if a cluster
    RVPV_ROOTINT   = (1ULL<<28),    // present at root level only
    RVPV_CMOMP     = (1ULL<<29),    // present if Zicbom/Zicbop implemented
    RVPV_CMOZ      = (1ULL<<30),    // present if Zicboz implemented
    RVPV_PMPINIT   = (1ULL<<31),    // present if PMP_initialparams is true
    RVPV_PMPMASK   = (1ULL<<32),    // present if PMP_romaskparams is true
    RVPV_CLEG      = (1ULL<<33),    // present if legacy C extension
    RVPV_CNEW      = (1ULL<<34),    // present if new C extension
    RVPV_CV07      = (1ULL<<35),    // present if C extension 0.70.x
    RVPV_WFI       = (1ULL<<36),    // requires true WFI (not NOP)
    RVPV_SMAIA     = (1ULL<<37),    // requires Smaia extension
    RVPV_ZAWRS     = (1ULL<<38),    // requires Zarws extension
    RVPV_TIMER     = (1ULL<<39),    // requires built-in timer
...
static vmiEnumParameter debugVariants[] = {
    ...
    },
    {
        .name        = "1.0-STABLE",
        .value       = RVDBG_1_0,
        .description = "RISC-V External Debug Support Version 1.0-STABLE as of "
                       "commit "RVDEBUG_SPEC_TAG" of "RVDEBUG_SPEC_DATE " "
...
static vmiEnumParameter rnmiVariants[] = {
    ...
    {
        .name        = "0.4_nmie1",
        .value       = RNMI_0_4_NMIE1,
        .description = "RNMI version 0.4, except mnstatus.nmie=1 at reset",
    },
    {
        .name        = "0.4",
        .value       = RNMI_0_4,
...
static vmiEnumParameter CLICVariants[] = {
    ...
    {
        .name        = "0.9-draft-20230801",
        .value       = RVCLC_0_9_20230801,
        .description = "CLIC Version 0.9-draft-20230801",
    },
    {
        .name        = "master",
        .value       = RVCLC_MASTER,
...
static vmiEnumParameter AIAVariants[] = {
    ...
    },
    {
        .name        = "1.0",
        .value       = RVAIA_1_0,
        .description = "AIA Ratified Version 1.0",
    },
    {
        .name        = "master",
        .value       = RVAIA_MASTER,
...
static vmiEnumParameter FSModes[] = {
    ...
    {
        .name        = "execute_not_store",
        .value       = RVFS_EXECUTE_NOT_S,
        .description = "Any floating point non-store instruction sets mstatus.fs dirty",
    },
    {
        .name        = "execute_any",
        .value       = RVFS_EXECUTE_ANY,
        .description = "Any floating point instruction sets mstatus.fs dirty",
    },
    {
        .name        = "always_dirty",
...
//
// Specify effect of illegal PMP updates with R=0 W=1
//
static vmiEnumParameter PMPR0W1Modes[] = {
    {
        .name        = "RWX_00X",
        .value       = RVPMPI_RWX_00X,
        .description = "set R=0 and W=0, modify X",
    },
    {
        .name        = "RWX_11X",
        .value       = RVPMPI_RWX_11X,
        .description = "set R=1 and W=1, modify X",
    },
    {
        .name        = "RWX_PPX",
        .value       = RVPMPI_RWX_PPX,
        .description = "preserve previous R and W, modify X",
    },
    {
        .name        = "RWX_PPP",
        .value       = RVPMPI_RWX_PPP,
        .description = "preserve previous RWX",
    },
    {
        .name        = "RWX_000",
        .value       = RVPMPI_RWX_000,
        .description = "set RWX=000",
    },
    // KEEP LAST: terminator
    {0}
};
...
//
// Set endian parameter default
//
static void setEndianParamDefault(vmiParameterP param, Uns32 value) {
    CHECK_PARAM_TYPE(param, vmi_PT_ENDIAN, "Endian");
...
//
// Macro to define a function to set a raw endian parameter value from the
// configuration
//
#define RISCV_ENDIAN_PDEFAULT_CFG_FN(_NAME) RISCV_PDEFAULT_FN(default_##_NAME) { \
    setEndianParamDefault(param, cfg->_NAME);  \
}
...
//
static RISCV_ENDIAN_PDEFAULT_CFG_FN(endian);
...
static RISCV_ENUM_PDEFAULT_CFG_FN(PMP_R0W1);
...
static RISCV_BOOL_PDEFAULT_CFG_FN(nmi_absent);
static RISCV_BOOL_PDEFAULT_CFG_FN(nmi_is_latched);
static RISCV_BOOL_PDEFAULT_CFG_FN(nmi_high_priority);
...
static RISCV_BOOL_PDEFAULT_CFG_FN(Sscofpmf);
...
//
// Specify default CSR remap string
//
static RISCV_PDEFAULT_FN(default_CSR_remap) {
    setStringParamDefault(param, cfg->CSR_remap ? : "");
}
...
//
// Get default value of tdata1_mask
//
static Uns64 getDefaultIPMask(riscvConfigCP cfg, Uns64 value, Uns64 defValue) {
...
static RISCV_PDEFAULT_FN(default_mip_mask) {
    Uns64 mask = getDefaultIPMask(cfg, cfg->csrMask.mip.u64.bits, WM32_mip);
    setUns64ParamDefault(param, mask);
}
static RISCV_PDEFAULT_FN(default_sip_mask) {
    Uns64 mask = getDefaultIPMask(cfg, cfg->csrMask.sip.u64.bits, WM32_sip);
    setUns64ParamDefault(param, mask);
}
static RISCV_PDEFAULT_FN(default_uip_mask) {
    Uns64 mask = getDefaultIPMask(cfg, cfg->csrMask.uip.u64.bits, WM32_uip);
    setUns64ParamDefault(param, mask);
}
static RISCV_PDEFAULT_FN(default_hip_mask) {
    Uns64 mask = getDefaultIPMask(cfg, cfg->csrMask.hip.u64.bits, WM32_hip);
    setUns64ParamDefault(param, mask);
}
...
static riscvParameter parameters[] = {
    ...
    {  RVPV_PRE,     0,         default_debug_mode,           VMI_ENUM_GROUP_PARAM_SPEC  (riscvParamValues, debug_mode,              DMModes,                   RV_GROUP(DBG),   "Specify how Debug mode is implemented (value 'none' implies Sdext is not implemented)")},
    ...
    {  RVPV_ALL,     0,         0,                            VMI_BOOL_GROUP_PARAM_SPEC  (riscvParamValues, wfi_restart,             False,                     RV_GROUP(ARTIF), "Specify whether to automatically restart from WFI state when model is simulated")},
    ...
    {  RVPV_NMI,     0,         default_ecode_nmi,            VMI_UNS64_GROUP_PARAM_SPEC (riscvParamValues, ecode_nmi,               0, 0,          -1,         RV_GROUP(INTXC), "Specify xcause.ExceptionCode for NMI")},
    ...
    {  RVPV_PRE,     0,         default_nmi_absent,           VMI_BOOL_GROUP_PARAM_SPEC  (riscvParamValues, nmi_absent,              False,                     RV_GROUP(INTXC), "Specify whether NMI input is absent")},
    {  RVPV_NMI,     0,         default_nmi_is_latched,       VMI_BOOL_GROUP_PARAM_SPEC  (riscvParamValues, nmi_is_latched,          False,                     RV_GROUP(INTXC), "Specify whether NMI input is latched on rising edge (if False, it is level-sensitive)")},
    {  RVPV_NMI,     0,         default_nmi_high_priority,    VMI_BOOL_GROUP_PARAM_SPEC  (riscvParamValues, nmi_high_priority,       False,                     RV_GROUP(INTXC), "Specify whether NMI input is higher priority than Debug and Trigger Module events")},
    ...
    {  RVPV_ALL,     0,         default_CSR_remap,            VMI_STRING_GROUP_PARAM_SPEC(riscvParamValues, CSR_remap,               0,                         RV_GROUP(ARTIF), "Comma-separated list of CSR number mappings, each of the form <csrName>=<number>")},
    ...
    {  RVPV_PRE,     0,         default_trigger_num,          VMI_UNS32_GROUP_PARAM_SPEC (riscvParamValues, trigger_num,             0, 0,          255,        RV_GROUP(TRIG),  "Specify the number of implemented hardware triggers (0 implies Sdtrig is not implemented)")},
    ...
    {  RVPV_NMI,     0,         default_nmi_address,          VMI_UNS64_GROUP_PARAM_SPEC (riscvParamValues, nmi_address,             0, 0,          -1,         RV_GROUP(INTXC), "Override NMI vector address")},
    ...
    {  RVPV_ALL,     0,         default_PMP_R0W1,             VMI_ENUM_GROUP_PARAM_SPEC  (riscvParamValues, PMP_R0W1,                PMPR0W1Modes,              RV_GROUP(PMP),   "Specify behavior of PMP RWX field on illegal write with R=0 and W=1")},
    ...
    {  RVPV_PRE,     0,         default_Sscofpmf,             VMI_BOOL_GROUP_PARAM_SPEC  (riscvParamValues, Sscofpmf,                False,                     RV_GROUP(EXT),   "Specify that Sscofpmf is implemented")},
    ...
    {  RVPV_ALL,     0,         default_endian,               VMI_ENDIAN_GROUP_PARAM_SPEC(riscvParamValues, endian,                                             RV_GROUP(FUND),  "Specify initial model endianness")},
...
static Bool selectParameter(
    ...
    if(cfg) {
        ...
        // include parameters that are only required when NMI is implemented
        if((param->variant & RVPV_NMI) && cfg->nmi_absent) {
            return False;
        }

        // include parameters that are only required when RNMI is implemented
        if((param->variant & RVPV_RNMI) && (cfg->nmi_absent || !cfg->rnmi_version)) {
            return False;
        }
...
VMI_SET_PARAM_VALUES_FN(riscvGetPreParamValues) {
    ...
    if(isSMPMember(riscv)) {
        ...
        // apply trigger_num override if required
        APPLY_PREPARAM_IF_SET(riscv, params, nmi_absent);
        ...
        // apply Sscofpmf override if required
        APPLY_PREPARAM_IF_SET(riscv, params, Sscofpmf);
