...
static void doTriggerAction(riscvP riscv, triggerAction action) {
    ...
    for(i=0; i<numTriggers(riscv); i++) {
        ...
        if(this->triggered) {
            ...
            Bool useThis = this->tvalValid;
            Bool isEPC   = (this->tval==epc);

            // use only first matching tval if required (default is last match)
            if(useThis && !(riscv->configInfo.chain_tval & RVCT_LAST)) {
                useThis = !tvalSet;
            }

            // prefer non-epc values for tval if required (since epc is reported
            // in xepc already)
            if(isEPC && (riscv->configInfo.chain_tval & RVCT_NON_EPC)) {
                useThis = False;
            }
