...
static const riscvExceptionDesc exceptions[] = {
    ...
    RISCV_EXCEPTION (NoException,                  0, ISA_C, "No exception (for exception query)"),
    RISCV_EXCEPTION (InstructionAddressMisaligned, 0, ISA_NC,"Fetch from unaligned address"),
    ...
    ////////////////////////////////////////////////////////////////////
    // COUNT OVERFLOW INTERRUPTS
    ////////////////////////////////////////////////////////////////////

    RISCV_EXCEPTION (LCOFIInterrupt,               0, 0,     "Count overflow interrupt"),
...
//
// Indicate whether inhv affects the EPC behavior in an exception
//
inline static Bool inhvAffectsEPC(riscvP riscv) {
    return riscv->configInfo.CLIC_version>RVCLC_0_9_20191208;
}

//
// Indicate whether xepc alignment is forced to XLEN/8 bytes on return from
// exception when inhv=1
//
inline static Bool inhvAlignEPC(riscvP riscv) {
    return riscv->configInfo.CLIC_version>=RVCLC_0_9_20230801;
}

//
// Indicate whether inhv is recorded in the target exception mode (if True) or
// the source interrupt mode (if False)
//
inline static Bool inhvInModeX(riscvP riscv) {
    return riscv->configInfo.CLIC_version>=RVCLC_0_9_20230801;
}
...
typedef struct trapCxtS {
    ...
    Bool        inhv;       // is a hardware vectoring trap?
} trapCxt, *trapCxtP;
...
//
// Update the value of inhv in the appropriate cause register
//
static void setTrapInhv(riscvP riscv, trapCxtP cxt) {

    riscvMode mode = inhvInModeX(riscv) ? cxt->modeX : cxt->modeY;

    if(mode==RISCV_MODE_U) {
        WR_CSR_FIELDC(riscv, ucause, inhv, cxt->inhv);
    } else if(mode==RISCV_MODE_S) {
        WR_CSR_FIELDC(riscv, scause, inhv, cxt->inhv);
    } else if(mode==RISCV_MODE_M) {
        WR_CSR_FIELDC(riscv, mcause, inhv, cxt->inhv);
    }
}
...
static void trapU(riscvP riscv, trapCxtP cxt) {
    ...
    // clear ucause register if not in CLIC mode and update inhv bit in either
    // modeX or modeY cause (depending on CLIC version)
    if(!useCLICU(riscv)) {
        WR_CSR64(riscv, ucause, 0);
    } else {
        setTrapInhv(riscv, cxt);
    }
...
static void trapVU(riscvP riscv, trapCxtP cxt) {
    ...
    // clear ucause register
    VMI_ASSERT(!useCLICVU(riscv), "VU-mode CLIC behavior currently undefined");
    WR_CSR64(riscv, ucause, 0);
...
static void trapHS(riscvP riscv, trapCxtP cxt) {
    ...
    // clear scause register if not in CLIC mode and update inhv bit in either
    // modeX or modeY cause (depending on CLIC version)
    if(!useCLICS(riscv)) {
...
static void trapVS(riscvP riscv, trapCxtP cxt) {
    ...
    VMI_ASSERT(!useCLICVS(riscv), "VU-mode CLIC behavior currently undefined");
...
static void trapM(riscvP riscv, trapCxtP cxt) {
    ...
    // clear mcause register if not in CLIC mode and update inhv bit in either
    // modeX or modeY cause (depending on CLIC version)
    if(!useCLICM(riscv)) {
...
static Bool getCLICVPC(riscvP riscv, Uns64 address, Uns64 *handlerPCP) {
    ...
    // mask table entry address to pointer size if required
    if(inhvAlignEPC(riscv)) {
        address &= getAddressMask(XLEN);
    }
...
void riscvTakeException(
    ...
    if(inDebugMode(riscv)) {
        ...
        trapCxt cxt = {
            handlerPC : 0,
            EPC       : riscv->inhv && inhvAffectsEPC(riscv) ? tval : getEPC(riscv),
        ...
        // clear CLIC vector access indication
        riscv->inhv = False;
...
static Uns32 fillSyndromeLoad(riscvInstrInfoP info, Uns32 offset) {
    ...
    u1.f.opcode = (isF ? 0x07 : 0x03);
    u1.f.rd     = getRIndex(rd);
    u1.f.funct3 = mapMemBits(info->memBits) + (info->unsExt*4);
    u1.f.offset = offset;

    // clear opcode bit 1 if a compressed instruction
    if(info->bytes==2) {
        u1.f.opcode &= ~2;
    }
...
//
// Get inhv for target mode when that is held in mode Y (legacy operation)
//
static Bool getXRETInhvModeY(riscvP riscv, riscvMode modeY) {

    Bool inhv = False;

    if(modeY==RISCV_MODE_U) {
        inhv = RD_CSR_FIELDC(riscv, ucause, inhv);
        WR_CSR_FIELDC(riscv, ucause, inhv, 0);
    } else if(modeY==RISCV_MODE_S) {
        inhv = RD_CSR_FIELDC(riscv, scause, inhv);
        WR_CSR_FIELDC(riscv, scause, inhv, 0);
    } else if(modeY==RISCV_MODE_M) {
...
void riscvMRET(riscvP riscv) {
    ...
    // perform CLIC-mode state updates
    if(useCLICM(riscv)) {

        // derive inhv value
        if(inhvInModeX(riscv)) {
            inhv = RD_CSR_FIELDC(riscv, mcause, inhv);
        } else if(inhvAffectsEPC(riscv)) {
            inhv = getXRETInhvModeY(riscv, newMode);
        }

        // restore previous mintstatus.mil
...
void riscvHSRET(riscvP riscv) {
    ...
    if(useCLICS(riscv)) {
        ...
        // restore previous mintstatus.sil
...
void riscvVSRET(riscvP riscv) {
    ...
    // CLIC behavior is not defined for VS mode
    VMI_ASSERT(!useCLICVS(riscv), "VS-mode CLIC behavior currently undefined");
...
void riscvURET(riscvP riscv) {
    ...
    if(useCLICU(riscv)) {
        ...
        // restore previous mintstatus.uil
...
static Int64 getDefaultPriBasic(riscvP riscv, Uns32 id) {
    ...
    if(result) {
    ...
    } else if(id!=exceptionToInt(riscv_E_LCOFIInterrupt)) {
        ...
        // priority of LCOFI depends on whether Sscofpmf is configured
        result = Sscofpmf(riscv) ? intPri[id] : 0;
    }
...
//
// Return an indication of whether interrupts are enabled
//
static Bool interruptsEnabled(riscvP riscv) {
    return (
        !inDebugMode(riscv) &&
...
static Bool getPendingAndEnabledNMI(riscvP riscv) {
    return RD_CSR_FIELDC(riscv, dcsr, nmip) && interruptsEnabled(riscv);
}
...
static Bool getPendingAndEnabled(riscvP riscv) {
    return (riscv->pendEnab.id!=RV_NO_INT) && interruptsEnabled(riscv);
}
...
VMI_IFETCH_FN(riscvIFetchExcept) {
    ...
    if(riscv->configInfo.nmi_high_priority && getPendingAndEnabledNMI(riscv)) {

        // handle pending high-priority NMI
        if(complete) {
            doNMI(riscv);
        }

    } else if(H_A && doHaltReq(riscv, complete)) {
    ...
    ////////////////////////////////////////////////////////////////////////////
        ...
        // handle pending low-priority NMI
        if(complete) {
            doNMI(riscv);
        }
...
Bool riscvHasStandardException(riscvP riscv, riscvException code) {
    ...
    if(code==riscv_E_NoException) {
        // exception with code 0 is always present (maybe as "No exception")
        return True;
    } else if(code==riscv_E_GenericNMI) {
        return nmiPresent(riscv) && !riscv->configInfo.ecode_nmi;
...
static riscvArchitecture getExceptionArch(riscvP riscv) {
    ...
    // determine whether misaligned instruction fetch exceptions are possible
    // (only if C extension is absent or can be disabled)
    if(!(arch&ISA_C) || (riscv->configInfo.archMask&ISA_C)) {
        arch &= ~ISA_C;
        arch |= ISA_NC;
    }

    // determine whether misaligned data access exceptions are possible
    if(
        ( riscv->configInfo.unaligned_check_ms)           ||
...
void riscvSetExceptionMask(riscvP riscv) {
    ...
    for(thisDesc=exceptions; thisDesc->vmiInfo.name; thisDesc++) {
        ...
        } else if(required&ISA_C) {
            // ignore artifact "No exception" placeholder
        } else if(code==riscv_E_GenericNMI) {
            // ignore generic NMI
        } else if((code==riscv_E_SGEIInterrupt) && !getGEILEN(riscv)) {
            // absent if GEILEN=0
        } else if((code==riscv_E_LCOFIInterrupt) && !Sscofpmf(riscv)) {
            // absent unless Sscofpmf present
        } else if((code==riscv_E_CSIP) && useCSIP12(riscv)) {
...
//
// Is resume from wait state required?
//
static Bool resumeFromWait(riscvP riscv) {

    Bool resume;

    if(riscv->disable & RVD_INT) {

        // resume if pending and enabled interrupt (custom model support)
        resume = riscvPendingAndEnabled(riscv);

    } else {

        // resume if pending locally enabled interrupt (maybe globally disabled)
        riscvBasicIntState intState;
...
// Is resume from wait state required?
//
Bool riscvResumeFromWait(riscvP riscv) {

    return (
        resumeFromWait(riscv) ||
        inDebugMode(riscv) ||
...
//
// Function called on expiry of artifact WFI restart timer (restart immediately
// after WFI)
//
static VMI_ICOUNT_FN(restartWFI) {
...
//
// Halt the passed processor in WFI state for the given reason (may be
// automatically restarted after one instruction)
//
static void haltWFI(riscvP riscv, riscvDisableReason reason) {

    riscvHalt(riscv, reason);

    if(riscv->wfiTimer) {
        vmirtSetModelTimer(riscv->wfiTimer, 0);
    }
}
...
void riscvWFI(riscvP riscv) {
    ...
    if(wfi_resume_not_trap && riscvResumeFromWait(riscv)) {
...
void riscvWRSSTO(riscvP riscv) {
    ...
    if(riscv->exclusiveTag==RISCV_NO_TAG) {
        ...
        haltWFI(riscv, RVD_WFI|RVD_WRS|RVD_STO);
        vmirtSetModelTimer(riscv->twTimer, riscv->configInfo.STO_time_limit);
    }
}

//
// Are there pending and enabled interrupts?
//
...
void riscvTestInterrupt(riscvP riscv) {
    ...
    } else if(riscv->disable && riscvResumeFromWait(riscv)) {
...
void riscvReset(riscvP riscv) {
    ...
    // call pre-reset notifier (in case it requires to log
    // current state before this is modified by standard reset actions)
    ITER_EXT_CB(
        riscv, extCB, preResetNotifier,
        extCB->preResetNotifier(riscv, extCB->clientData);
    )

    // enable interrupts that would be blocked by RNMI state if RNMI is absent,
    // otherwise disable those interrupts if RNMI version is 0.4 or higher
    // (mnstatus.NMIE explicitly resets to 0 from that version, but otherwise
    // reset value is undefined; special version RNMI_0_4_NMIE1 is like 0.4
    // but resets the field to 1)
    WR_CSR_FIELDC(riscv, mnstatus, NMIE, RISCV_RNMI_VERSION(riscv)<RNMI_0_4);
    ...
    // perform reset notifier (allows the derived model to modify any standard
    // reset state settings applied by the base model)
    ITER_EXT_CB(
...
static void doNMI(riscvP riscv) {
    ...
    ITER_EXT_CB(
        ...
        {
            // indicate mcause/mncause has been set by custom NMI callback
            causeSet = True;

            // invoke custom NMI handler (which must set mcause/mncause) and
            // terminate before performing standard actions if required
    ...
    if(RISCV_RNMI_VERSION(riscv)) {
        ...
        // RNMI implemented: get any custom mncause setting
        Uns64 mncause = causeSet ? RD_CSR_M(riscv, mncause) : 0;

        // disable level-sensitive NMI
        WR_CSR_FIELDC(riscv, mnstatus, NMIE, 0);

        // update mncause register, preserving non-zero bits set by custom NMI
        WR_CSR_M(riscv, mncause, ecode_nmi | mncause);
        ...
        // RNMI not implemented: get any custom mcause setting
        Uns64 mcause = causeSet ? RD_CSR_M(riscv, mcause) : 0;

        // nmi input is edge sensitive and pending NMI must be cleared here
        WR_CSR_FIELDC(riscv, dcsr, nmip, 0);

        // update mcause register, preserving non-zero bits set by custom NMI
        WR_CSR_M(riscv, mcause, ecode_nmi | mcause);
...
//
// Update state on change of effective NMI state
//
static void refreshNMI(riscvP riscv) {

    Bool oldValue = RD_CSR_FIELDC(riscv, dcsr, nmip);
    Bool newValue = riscv->NMI && !riscv->disableNMI;
...
//
// Update externally-disabled NMI state
//
void riscvUpdateInterruptDisableNMI(riscvP riscv, Bool disable) {

    riscv->disableNMI = disable;
...
void riscvNewNetPorts(riscvP riscv) {
    ...
    tail = newNetPortsTemplate(riscv, tail, RVP_reset, RVP_reset_addr);

    // add NMI ports if required
    if(nmiPresent(riscv)) {
        tail = newNetPortsTemplate(riscv, tail, RVP_nmi, RVP_nmi_addr);
    }
    ...
    if(nmiPresent(riscv) && RISCV_RNMI_VERSION(riscv)) {
...
void riscvNewTimers(riscvP riscv) {
    ...
    if(riscv->restartFromWFI) {
        riscv->wfiTimer = newCycleTimer(riscv, restartWFI);
    }
}

//
// Free a timer if it is allocated
//
inline static void freeTimer(vmiModelTimerPP timerP) {
    if(*timerP) {
        vmirtDeleteModelTimer(*timerP);
        *timerP = 0;
    }
}
...
void riscvFreeTimers(riscvP riscv) {
    freeTimer(&riscv->mtime);
    freeTimer(&riscv->stepTimer);
    freeTimer(&riscv->twTimer);
    freeTimer(&riscv->ackTimer);
    freeTimer(&riscv->wfiTimer);
}
...
void riscvTimerSave(
    ...
    if(phase==SRT_END_CORE) {
        ...
        if(riscv->wfiTimer) {
            vmirtSaveModelTimer(cxt, RV_WFI_TIMER, riscv->wfiTimer);
        }
    }
}
...
void riscvTimerRestore(
    ...
    if(phase==SRT_END_CORE) {
        ...
        if(riscv->wfiTimer) {
            vmirtRestoreModelTimer(cxt, RV_WFI_TIMER, riscv->wfiTimer);
        }
    }
}
