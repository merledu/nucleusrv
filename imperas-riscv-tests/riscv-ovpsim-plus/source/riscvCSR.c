...
inline static RISCV_CSR_PRESENTFN(rnmi04P) {
    return RISCV_RNMI_VERSION(riscv)>=RNMI_0_4_NMIE1;
}
...
static RISCV_CSR_READFN(mncauseR) {
    ...
    Uns64 result = RD_CSR(riscv, mncause);

    return result | mncauseInterrupt(riscv);
}
...
static RISCV_CSR_WRITEFN(mncauseW) {
    ...
    newValue &= riscv->configInfo.ecode_nmi_mask;

    WR_CSR(riscv, mncause, newValue);
...
static RISCV_CSR_WRITEFN(mnstatusW) {
    ...
    // revert mnstatus.MNPP if target mode is not implemented
    if(!riscvHasMode(riscv, newMPP)) {
        WR_CSR_FIELDC(riscv, mnstatus, MNPP, oldMPP);
    }

    // mnstatus.NMIE may only be set by software (not cleared)
    if(oldMIE && !newMIE && !riscv->artifactAccess) {
        WR_CSR_FIELDC(riscv, mnstatus, NMIE, oldMIE);
    }

    // handle interrupts that may have been enabled
    if(!oldMIE && newMIE) {
        riscvTestInterrupt(riscv);
    }
...
//
// Is Sscofpmf extension present?
//
inline static RISCV_CSR_PRESENTFN(SscofpmfP) {
    return Sscofpmf(riscv) && !riscv->configInfo.mhpmcounter_undefined;
}
...
//
// Return index of event given CSR number
//
inline static Uns32 getEventStateIndex(riscvCSRAttrsCP attrs) {
    return attrs->csrNum & 31;
}

//
// Update value of event state for the given CSR
//
static void setEventState(riscvCSRAttrsCP attrs, riscvP riscv, Uns8 newValue) {
    riscv->eventState[getEventStateIndex(attrs)] = newValue & riscv->eventState[0];
}

//
// Return value of event state for the given CSR
//
static Uns64 getEventState(riscvCSRAttrsCP attrs, riscvP riscv) {
    return riscv->eventState[getEventStateIndex(attrs)];
}

//
// Return value of mhpmevent when XLEN=64
//
inline static Uns64 getEventState64(riscvCSRAttrsCP attrs, riscvP riscv) {
    return getEventState(attrs, riscv) << 56;
}

//
// Return value of mhpmeventh when XLEN=32
//
inline static Uns32 getEventState32(riscvCSRAttrsCP attrs, riscvP riscv) {
    return getEventState(attrs, riscv) << 24;
}

//
// Read mhpmevent register (unimplemented except for Sscofpmf bits)
//
static RISCV_CSR_READFN(mhpmeventR) {
    ...
    if(!riscvHPMAccessValid(attrs, riscv)) {
        // no action
    } else if(!RISCV_XLEN_IS_32M(riscv, getCSRMode4(attrs, riscv))) {
        result = getEventState64(attrs, riscv);
    }
...
//
// Write mhpmevent register (unimplemented except for Sscofpmf bits)
//
static RISCV_CSR_WRITEFN(mhpmeventW) {
    ...
    if(!riscvHPMAccessValid(attrs, riscv)) {
        ...
        setEventState(attrs, riscv, newValue>>56);
    }

    return getEventState64(attrs, riscv);
}

//
// Read mhpmeventh register (unimplemented except for Sscofpmf bits)
//
static RISCV_CSR_READFN(mhpmeventhR) {

    Uns32 result = 0;

    if(riscvHPMAccessValid(attrs, riscv)) {
        result = getEventState32(attrs, riscv);
    }
...
//
// Write mhpmeventh register (unimplemented except for Sscofpmf bits)
//
static RISCV_CSR_WRITEFN(mhpmeventhW) {

    if(riscvHPMAccessValid(attrs, riscv)) {
        setEventState(attrs, riscv, newValue>>24);
    }

    return getEventState32(attrs, riscv);
}

//
// Read scountovf register
//
static RISCV_CSR_READFN(scountovfR) {

    Uns32 result = 0;
    Uns32 i;

    // compose result from OV bits of mhpmevent3-31
    for(i=3; i<32; i++) {
        result |= (riscv->eventState[i]>>7)<<i;
    }

    // read access is always mediated by mcounteren
    result &= RD_CSRC(riscv, mcounteren);

    // in VS mode, read access is further mediated by hcounteren
    if(inVMode(riscv)) {
        result &= RD_CSRC(riscv, hcounteren);
    }
...
//
// Is the disabled trigger (type 15) supported?
//
inline static Bool supportDisabled(riscvTriggerP trigger) {
    return RD_REG_TRIGGER(trigger, tinfo) & (1<<TT_DISABLED);
}
...
static Bool validateTriggerType(
    ...
    Uns32       tinfo = RD_REG_TRIGGER(trigger, tinfo);
    Bool        ok    = False;

    if(tinfo & (1<<type)) {

        // trigger type is explicitly supported
        ok = True;

    } else if(!value && supportDisabled(trigger)) {

        // trigger type 0 write is legal if disabled type (15) is supported
        ok = True;

    } else if(riscv->verbose) {
...
static riscvTData1UP unpackTData1(
    ...
    switch(tdata1UP.type) {
        ...
        case TT_NONE:
            ...
            // writing 0 to the trigger should force it to a disabled state
            // if that trigger type is supported
            if(supportDisabled(trigger)) {
                tdata1UP.type = TT_DISABLED;
            }
        ...
        default:
            ...
            // explicit write of disabled trigger (type 15) ignores all
            // remaining bits except dmode
            break;
    }
...
static Uns64 packTData1(
    ...
    switch(tdata1UP.type) {
        ...
        default:
            ...
            // no action for other trigger types (e.g. TT_DISABLED)
            break;
    }
...
//
// Adjust legal tdata2 value when type 6 trigger is active - for this trigger
// type, if bits maskmax6-1:0 are all ones then those bits are forced to zero
//
static Uns64 tdata2Adjust6(riscvP riscv, Uns64 value) {

    Uns64 allOnes   = TRIGGER_IS_32M(riscv) ? (Uns32)-1 : (Uns64)-1;
    Uns64 hiBitMask = allOnes & (-1ULL<<riscv->configInfo.mcontrol_maskmax);
    Uns64 loBitMask = allOnes & ~hiBitMask;

    if((value & loBitMask) == loBitMask) {
        value &= hiBitMask;
    }
...
static RISCV_CSR_READFN(tdata2R) {
    ...
    if(riscv->artifactAccess) {
        ...
        result = tdata2Adjust6(riscv, result);
...
static RISCV_CSR_WRITEFN(tdata2W) {
    ...
    if(mayWriteTrigger(riscv, trigger)) {
        ...
        if(riscv->artifactAccess) {
            ...
            // limit value to mcontrol_maskmax
            newValue = tdata2Adjust6(riscv, newValue);
...
//
// Implemented using callbacks only, high half, numbers 0..9
//
#define CSR_ATTR_PH_0_9( \
    _ID, _NUM, _ARCH, _ACCESS, _VERSION,                                \
    _ENDB,_TMODE,_TRAP,_WRD,_NOSR,_V,_STATEEN,                          \
    _DESC, _PRESENT, _WSTATE, _RCB, _RWCB, _WCB                         \
) \
    ...
    CSR_ATTR_PH_NUM(_ID, _NUM, 2, _ARCH, _ACCESS, _VERSION, _ENDB,_TMODE,_TRAP,_WRD,_NOSR,_V,_STATEEN, _DESC, _PRESENT, _WSTATE, _RCB, _RWCB, _WCB), \
    CSR_ATTR_PH_NUM(_ID, _NUM, 3, _ARCH, _ACCESS, _VERSION, _ENDB,_TMODE,_TRAP,_WRD,_NOSR,_V,_STATEEN, _DESC, _PRESENT, _WSTATE, _RCB, _RWCB, _WCB), \
    CSR_ATTR_PH_NUM(_ID, _NUM, 4, _ARCH, _ACCESS, _VERSION, _ENDB,_TMODE,_TRAP,_WRD,_NOSR,_V,_STATEEN, _DESC, _PRESENT, _WSTATE, _RCB, _RWCB, _WCB), \
    CSR_ATTR_PH_NUM(_ID, _NUM, 5, _ARCH, _ACCESS, _VERSION, _ENDB,_TMODE,_TRAP,_WRD,_NOSR,_V,_STATEEN, _DESC, _PRESENT, _WSTATE, _RCB, _RWCB, _WCB), \
    CSR_ATTR_PH_NUM(_ID, _NUM, 6, _ARCH, _ACCESS, _VERSION, _ENDB,_TMODE,_TRAP,_WRD,_NOSR,_V,_STATEEN, _DESC, _PRESENT, _WSTATE, _RCB, _RWCB, _WCB), \
    CSR_ATTR_PH_NUM(_ID, _NUM, 7, _ARCH, _ACCESS, _VERSION, _ENDB,_TMODE,_TRAP,_WRD,_NOSR,_V,_STATEEN, _DESC, _PRESENT, _WSTATE, _RCB, _RWCB, _WCB), \
    CSR_ATTR_PH_NUM(_ID, _NUM, 8, _ARCH, _ACCESS, _VERSION, _ENDB,_TMODE,_TRAP,_WRD,_NOSR,_V,_STATEEN, _DESC, _PRESENT, _WSTATE, _RCB, _RWCB, _WCB), \
    CSR_ATTR_PH_NUM(_ID, _NUM, 9, _ARCH, _ACCESS, _VERSION, _ENDB,_TMODE,_TRAP,_WRD,_NOSR,_V,_STATEEN, _DESC, _PRESENT, _WSTATE, _RCB, _RWCB, _WCB)

//
// Implemented using callbacks only, high half only, numbers 3..31
//
#define CSR_ATTR_PH_3_31( \
    _ID, _NUM, _ARCH, _ACCESS, _VERSION,                                \
    _ENDB,_TMODE,_TRAP,_WRD,_NOSR,_V,_STATEEN,                          \
    _DESC, _PRESENT, _WSTATE, _RCB, _RWCB, _WCB                         \
) \
    ...
    CSR_ATTR_PH_NUM(_ID,    _NUM, 8, _ARCH, _ACCESS, _VERSION, _ENDB,_TMODE,_TRAP,_WRD,_NOSR,_V,_STATEEN, _DESC,    _PRESENT, _WSTATE, _RCB, _RWCB, _WCB), \
    CSR_ATTR_PH_NUM(_ID,    _NUM, 9, _ARCH, _ACCESS, _VERSION, _ENDB,_TMODE,_TRAP,_WRD,_NOSR,_V,_STATEEN, _DESC,    _PRESENT, _WSTATE, _RCB, _RWCB, _WCB), \
    CSR_ATTR_PH_0_9(_ID##1, _NUM+10, _ARCH, _ACCESS, _VERSION, _ENDB,_TMODE,_TRAP,_WRD,_NOSR,_V,_STATEEN, _DESC"1", _PRESENT, _WSTATE, _RCB, _RWCB, _WCB), \
    CSR_ATTR_PH_0_9(_ID##2, _NUM+20, _ARCH, _ACCESS, _VERSION, _ENDB,_TMODE,_TRAP,_WRD,_NOSR,_V,_STATEEN, _DESC"2", _PRESENT, _WSTATE, _RCB, _RWCB, _WCB), \
    CSR_ATTR_PH_NUM(_ID,    _NUM,30, _ARCH, _ACCESS, _VERSION, _ENDB,_TMODE,_TRAP,_WRD,_NOSR,_V,_STATEEN, _DESC,    _PRESENT, _WSTATE, _RCB, _RWCB, _WCB), \
    CSR_ATTR_PH_NUM(_ID,    _NUM,31, _ARCH, _ACCESS, _VERSION, _ENDB,_TMODE,_TRAP,_WRD,_NOSR,_V,_STATEEN, _DESC,    _PRESENT, _WSTATE, _RCB, _RWCB, _WCB)
...
static const riscvCSRAttrs csrs[CSR_ID(LAST)] = {
    ...
    CSR_ATTR_P__     (scountovf,    0xDA0, ISA_S,       0,          1_10,   0,0,0,0,0,0, 0,                    "Supervisor Count Overflow",                             SscofpmfP,       0,        scountovfR,    0,        0             ),
    ...
    CSR_ATTR_P__3_31 (mhpmevent,    0x320, 0,           0,          1_10,   0,0,0,0,0,0, 0,                    "Machine Performance Monitor Event Select ",             mhpmcounterP, 0,           mhpmeventR,    0,        mhpmeventW    ),
    CSR_ATTR_PH_3_31 (mhpmevent,    0x720, ISA_32,      0,          1_10,   0,0,0,0,0,0, 0,                    "Machine Performance Monitor Event Select High ",        SscofpmfP,    0,           mhpmeventhR,   0,        mhpmeventhW   ),
...
void riscvCSRInit(riscvP riscv) {
    ...
    //--------------------------------------------------------------------------
    // initialize Sscofpmf event state mask
    //--------------------------------------------------------------------------

    if(Sscofpmf(riscv)) {

        // OF and MINH always present
        Uns8 eventMask = 0xc0;

        // SINH, UINH, VSINH and VUINH depend on feature presence
        if(arch&ISA_S) {eventMask |= 0x20;}
        if(arch&ISA_U) {eventMask |= 0x10;}
        if(arch&ISA_H) {eventMask |= 0x0c;}

        // use entry zero for mask
        riscv->eventState[0] = eventMask;
    }
}
...
//
static Bool invalidDebugCSRAccess(
    riscvP          riscv,
    riscvCSRAttrsCP attrs,
    Uns32           csrNum
) {
    Bool debugOnly = IS_DEBUG_CSR(csrNum) || (attrs && (attrs->trap & CSRT_DBG));
    return debugOnly && !inDebugMode(riscv);
}
...
riscvCSRAttrsCP riscvValidateCSRAccess(
    ...
    } else if(invalidDebugCSRAccess(riscv, attrs, csrNum)) {
