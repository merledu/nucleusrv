...
//
// Construct enumeration member names from the given base and indices 0..9 + h
//
#define CSR_ID_0_9H(_R) \
    CSR_ID(_R##0##h),   \
    CSR_ID(_R##1##h),   \
    CSR_ID(_R##2##h),   \
    CSR_ID(_R##3##h),   \
...
//
// Construct enumeration member names from the given base and indices 3..31 + h
//
#define CSR_ID_3_31H(_R) \
    CSR_ID(_R##0##h),   \
    CSR_ID(_R##1##h),   \
    CSR_ID(_R##2##h),   \
    CSR_ID(_R##3##h),   \
...
typedef enum riscvCSRIdE {
    ...
    CSR_ID_3_31H(mhpmevent),    // 0x723-0x73F
...
typedef struct {
    ...
    Uns32 MINH  :  1;
    Uns32 OF    :  1;
} CSR_REG_TYPE_64(hpmevent);

// define 64 bit type
CSR_REG_STRUCT_DECL_64(hpmevent);

typedef CSR_REG_TYPE(hpmevent) CSR_REG_TYPE(mhpmevent);
...
typedef enum triggerTypeE {
    ...
    TT_DISABLED    = 15,    // present but disabled
    TT_UNAVAILABLE = 15,    // (legacy entry)
} triggerType;
