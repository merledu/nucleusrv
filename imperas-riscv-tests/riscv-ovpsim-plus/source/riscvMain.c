...
static void initLeafModelCBs(riscvP riscv) {
    ...
    riscv->cb.updateDisableNMI   = riscvUpdateInterruptDisableNMI;
    riscv->cb.testInterrupt      = riscvUpdatePending;
    riscv->cb.resumeFromWFI      = riscvResumeFromWait;
...
static void applyParamsSMP(
    ...
    riscv->restartFromWFI = params->wfi_restart;

    // set data endian (instruction fetch is always little-endian)
    cfg->endian = params->endian;
    ...
    // get implied CSR sign extension configuration parameters
    riscv->mtvec_sext = getSExtendBits(params->mtvec_sext, cfg->csrMask.mtvec.u64.bits);
    riscv->stvec_sext = getSExtendBits(params->stvec_sext, cfg->csrMask.stvec.u64.bits);
    riscv->utvec_sext = getSExtendBits(params->utvec_sext, cfg->csrMask.utvec.u64.bits);
    riscv->mtvt_sext  = getSExtendBits(params->mtvt_sext,  cfg->csrMask.mtvt.u64.bits);
    riscv->stvt_sext  = getSExtendBits(params->stvt_sext,  cfg->csrMask.stvt.u64.bits);
    riscv->utvt_sext  = getSExtendBits(params->utvt_sext,  cfg->csrMask.utvt.u64.bits);
    ...
    cfg->nmi_absent           = params->nmi_absent;
    cfg->nmi_is_latched       = params->nmi_is_latched;
    cfg->nmi_high_priority    = params->nmi_high_priority;
    ...
    cfg->archImplicit    &= ~riscvParseExtensions(params->sub_implicit_Extensions);
    ...
    cfg->archImplicit    |=  riscvParseExtensions(params->add_implicit_Extensions);

    // derive list of all enabled or potentially enabled implicit extensions
    riscvArchitecture implicit_Extensions = extensionsPresent(
        cfg, misa_Extensions, riscvGetImplictExtensionMask(cfg)
    );
    ...
    ADD_K_SET(riscv, cfg, params, Zvbb);
    ...
    // Zvbb implies Zvkb for feature presence
    if(!(cfg->crypto_absent & RVKS_Zvbb)) {
        cfg->crypto_absent &= ~RVKS_Zvkb;
    }
...
VMI_SAVE_STATE_FN(riscvSaveState) {
    ...
    switch(phase) {
        ...
        case SRT_END_CORE:
            ...
            VMIRT_SAVE_FIELD(cxt, riscv, disableNMI);
            break;
...
VMI_RESTORE_STATE_FN(riscvRestoreState) {
    ...
    switch(phase) {
        ...
        case SRT_END_CORE:
            ...
            VMIRT_RESTORE_FIELD(cxt, riscv, disableNMI);
