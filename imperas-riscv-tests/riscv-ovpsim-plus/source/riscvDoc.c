...
static void listFeatures(
    ...
    riscvArchitecture implicit = riscvGetImplictExtensionMask(cfg);
    Uns32             featureBit;
    ...
    for(featureBit = 0; featureBit <= ('Z'-'A'); featureBit++) {
        ...
        if(arch & feature) {
            ...
            snprintf(
                string, stringLen,
                "%s bit %u: %s",
                (feature&implicit) ? "implicit feature" : "misa",
                featureBit,
...
static void docCLIC(riscvP riscv, vmiDocNodeP Root) {
    ...
    if(riscv->parent) {
        ...
        ////////////////////////////////////////////////////////////////////////
        // CLIC VERSION 0.9-draft-20230801
        ////////////////////////////////////////////////////////////////////////

        {
            ...
            vmidocAddText(
                Version,
                "Stable 0.9 version of August 1 2023, with these changes "
                "compared to version 0.9-draft-20221108:"
            );
            vmidocAddText(
                Version,
            ...
            vmidocAddText(
                Version,
                "- xepc is forced to table-entry alignment on xRET when inhv "
                "is active;"
            );
            vmidocAddText(
                Version,
...

        {
            ...
            vmidocAddText(
                ...
                RVCLC_MASTER_TAG"), currently identical to version "
                "0.9-draft-20230801."
            );
        }
    }
...
static void docAIA(riscvP riscv, vmiDocNodeP Root) {
    ...
    if(riscv->parent) {
...

        {
            ...
            vmidocAddText(
                Version,
                "1.0 ratified version of July 1 2023,  functionally identical "
                "to 1.0-RC5."
            );
        }
...

        {
            ...
            vmidocAddText(
                ...
                RVAIA_MASTER_TAG"), currently identical to 1.0."
            );
        }
    }
}
...
static vmiDocNodeP docSMP(riscvP rootProcessor) {
...

    {
        ...
        riscvArchitecture implicitMask = riscvGetImplictExtensionMask(cfg);
        ...
        listFeatures(cfg, EnabledExt, archExplicit, SNPRINTF_TGT(string));
        ...
        if(archImplicit) {
            ...
            listFeatures(cfg, EnabledExt, archImplicit, SNPRINTF_TGT(string));
        }
        ...
        if(canEnable) {
            ...
            riscvArchitecture canEnableI = canEnable &  implicitMask;
            riscvArchitecture canEnableE = canEnable & ~implicitMask;
            ...
            if(canEnableE) {
                ...
                vmidocAddText(
                    ExtSubset,
                    "The following misa-visible extensions are supported by "
                    "the model, but not enabled by default in this variant:"
                );

                // report the extensions that are supported but not enabled
                listFeatures(cfg, ExtSubset, canEnableE, SNPRINTF_TGT(string));

                vmidocAddText(
                    ExtSubset,
                    "To add features from this list to the visible set in the "
                    "misa register, use parameter \"add_Extensions\". This is "
                    "a string containing identification letters of features to "
                    "enable; for example, value \"DV\" indicates that "
                    "double-precision floating point and the Vector Extension "
                    "should be enabled, if they are currently absent and are "
                    "available on this variant."
                );

                vmidocAddText(
                    ExtSubset,
                    "Legacy parameter \"misa_Extensions\" can also be used. "
                    "This Uns32-valued parameter specifies the reset value for "
                    "the misa CSR Extensions field, replacing any permitted "
                    "bits defined in the base variant."
                );
            }
            ...
            if(canEnableI) {
                ...
                vmidocAddText(
                    ExtSubset,
                    "The following implicit extensions are supported by the "
                    "model, but not enabled by default in this variant:"
                );

                // report the extensions that are supported but not enabled
                listFeatures(cfg, ExtSubset, canEnableI, SNPRINTF_TGT(string));

                vmidocAddText(
                    ExtSubset,
                    "To add features from this list to the implicitly-enabled "
                    "set (not visible in the misa register), use parameter "
                    "\"add_implicit_Extensions\". This is a string parameter "
                    "in the same format as the \"add_Extensions\" parameter "
                    "described above."
                );
            }
        }
        ...
        if(canDisable) {
            ...
            riscvArchitecture canDisableI = canDisable &  implicitMask;
            riscvArchitecture canDisableE = canDisable & ~implicitMask;
            ...
            if(canDisableE) {
                ...
                vmidocAddText(
                    ExtSubset,
                    "The following misa-visible extensions are enabled by "
                    "default in the model and can be disabled:"
                );

                // report the extensions that are enabled and can be disabled
                listFeatures(cfg, ExtSubset, canDisableE, SNPRINTF_TGT(string));

                vmidocAddText(
                    ExtSubset,
                    "To disable features that are enabled by default, use "
                    "parameter \"sub_Extensions\". This is a string containing "
                    "identification letters of features to disable; for "
                    "example, value \"DF\" indicates that double-precision and "
            ...
            if(canDisableI) {
                ...
                vmidocAddText(
                    ExtSubset,
                    "The following implicit extensions are enabled by default "
                    "in the model and can be disabled:"
                );

                // report the extensions that are supported but not enabled
                listFeatures(cfg, ExtSubset, canDisableI, SNPRINTF_TGT(string));

                vmidocAddText(
                    ExtSubset,
                    "To remove features from this list from the "
                    "implicitly-enabled set (not visible in the misa "
                    "register), use parameter \"sub_implicit_Extensions\". "
                    "This is a string parameter in the same format as the "
                    "\"sub_Extensions\" parameter described above."
                );
            }
        }
    }
...

    {
        ...
        // document mtvec CSR behavior
        {
            ...
            if(cfg->mtvec_is_ro) {
                ...
                addDocSExt(sub, riscv->mtvec_sext, "mtvec", "mtvec_sext");
            }
        ...
        if(cfg->arch&ISA_S) {
            ...
            addDocSExt(sub, riscv->stvec_sext, "stvec", "stvec_sext");
        }
        ...
        if(cfg->arch&ISA_N) {
            ...
            addDocSExt(sub, riscv->utvec_sext, "utvec", "utvec_sext");
        }
        ...
        // document NMI behavior
        {
            ...
            if(cfg->nmi_absent) {
                ...
                vmidocAddText(
                    sub,
                    "An NMI input is not implemented. To instead specify that "
                    "NMI is implemented, set parameter \"nmi_absent\" to False."
                );
                ...
                vmidocAddText(
                    sub,
                    "An NMI input is implemented. To instead specify that NMI "
                    "is not implemented, set parameter \"nmi_absent\" to True."
                );

                snprintf(
                    SNPRINTF_TGT(string),
                    "On an NMI, the model will restart at address 0x"FMT_Ax"; "
                    "a different NMI address may be specified using parameter "
                    ...
                    "using parameter \"ecode_nmi\" or applied using optional "
                    "input port \"nmi_cause\" if required.",
                ...
                snprintf(
                    ...
                    "NMIs are supported, managed by additional CSRs "
                    "\"mnscratch\", \"mnepc\", \"mncause\" and \"mnstatus\", "
                    "following the indicated version of the Resumable NMI "
                    "extension proposal. In this variant, \"rnmi_version\" is "
                    "\"%s\".",
                ...
                if(cfg->nmi_is_latched) {
                    vmidocAddText(
                        sub,
                        "The NMI input is latched on the rising edge of the "
                        "NMI signal. To instead specify that NMI input is "
                        "level-sensitive, set parameter \"nmi_is_latched\" to "
                        "False."
                    );
                } else {
                    vmidocAddText(
                        sub,
                        "The NMI input is level-sensitive. To instead specify "
                        "that the NMI input is latched on the rising edge of "
                        "the NMI signal, set parameter \"nmi_is_latched\" to "
                        "True."
                    );
                }

                if(cfg->nmi_high_priority) {
                    vmidocAddText(
                        sub,
                        "NMI interrupts are higher priority than Debug and "
                        "Trigger Module events. To instead specify that NMI "
                        "interrupts are lower priority, set parameter "
                        "\"nmi_high_priority\" to False."
                    );
                } else {
                    vmidocAddText(
                        sub,
                        "NMI interrupts are lower priority than Debug and "
                        "Trigger Module events. To instead specify that NMI "
                        "interrupts are higher priority, set parameter "
                        "\"nmi_high_priority\" to True."
                    );
                }
            }
        }
    ...
    if(cfg->arch&ISA_DF) {
        ...
        if(cfg->Zfinx_version) {
            ...
            vmidocAddText(
                ...
                "will be set to 3 (Dirty) by any explicit write to the fflags, "
                "frm or fcsr control registers, or by any executed instruction "
                "that writes an FPR, or by any executed floating point compare "
                "or conversion even if those instructions do not signal a "
                "floating point exception."
            );
            vmidocAddText(
                Features,
                "If \"mstatus_fs_mode\" is set to \"execute_not_store\" then "
                "mstatus.FS will be set to 3 (Dirty) when any floating point "
                "instruction except a floating point store is executed. This "
                "includes all instructions selected by \"write_any\" mode, and "
                "also floating point moves targeting GPRs."
            );
            vmidocAddText(
                Features,
                "If \"mstatus_fs_mode\" is set to \"execute_any\" then "
                "mstatus.FS will be set to 3 (Dirty) when any floating point "
                "instruction is executed. This includes all instructions "
                "selected by \"execute_not_store\" mode, and also floating "
                "point stores."
            );
...

    {
        ...
        {
            static const char *meanings[] = {
                "count overflow and mode-based filtering extension CSRs are "
                "undefined",
                "count overflow and mode-based filtering extension CSRs are "
                "defined",
            };
            ...
            vmiDocNodeP sub = docBoolParam(
                extP, "Sscofpmf", cfg->Sscofpmf, meanings
            );

            vmidocAddText(
                sub,
                "Note that this model implements CSR state only for this "
                "extension; hardware performance counters themselves are "
                "implementation-specific and not implemented."
            );
        }
    ...
    {
        vmiDocNodeP debugMode = vmidocAddSection(Root, "Debug Mode (Sdext)");
        ...
        vmidocAddText(
            debugMode,
            "Note that parameter \"debug_mode\" can be also be set to \"none\" "
            "to specify that Sdext is not implemented."
        );
    ...
    if(cfg->trigger_num) {
        ...
        vmiDocNodeP trigger = vmidocAddSection(Root, "Trigger Module (Sdtrig)");
...

        {
            ...
            snprintf(
                ...
                "Set this parameter to 0 to specify that Sdtrig is not "
                "implemented.",
